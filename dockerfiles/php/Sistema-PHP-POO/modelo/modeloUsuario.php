...
class modeloUsuario {
	...
	function modificar($id,$nombre,$clave,$activo,$mod01,$mod02,$mod03) 
	{
		try {
			$MD5clave = md5($clave);
			...
			$stm = $this->pdo->prepare($sql);
			$resultSet = $stm->execute([$nombre,$MD5clave,$activo,$mod01,$mod02,$mod03,$id]);
		
		} catch(Exception $e) {
			die($e->getMessage());
		}
	}
	function modificar2($id,$nombre,$activo,$mod01,$mod02,$mod03) 
	{
		try {
			$MD5clave = md5($clave);
			$sql  = "UPDATE usuario SET 
	...
	function insertar($id,$nombre,$clave,$activo,$mod01,$mod02,$mod03) 
	{
		try {
			$MD5clave = md5($clave);
			$sql  = "INSERT INTO usuario (id_usu,descrip_usu,clave_usu,activo,
					modulo01, modulo02, modulo03) VALUES (?,?,?,?,
					?,?,?) ";
			$stm = $this->pdo->prepare($sql);
			$resultSet = $stm->execute([$id,$nombre,$MD5clave,$activo,$mod01,$mod02,$mod03]);
